...
class ReprintVoidReceiptService {
  final PrinterManager printerManager = PrinterManager.instance;

  Future<void> printReceipt({
    required ReprintVoidTransactionResponse transaction,
  }) async {
  ...
  Future<void> _printReceiptToDevice(
    ...
    ReprintVoidTransactionResponse transaction,
    Customer? customer,
  ) async {
    ...
    if (transaction.discountedItems.isNotEmpty) {
      ...
      bytes += generator.row([
        ...
        PosColumn(
          text: '- P ${transaction.transactionDetails.vatAdjustments}',
          width: 5,
          styles: PosStyles(align: PosAlign.right),
        ),
      ]);
